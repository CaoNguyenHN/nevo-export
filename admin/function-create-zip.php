 ...
 * @since    1.0.0
 */
if ( ! function_exists( 'nevo_export_create_zip' ) ) {
	function nevo_export_create_zip( $source, $wp_filesystem ) {

		/*Check if Zip Extension Installed*/
		if ( ! class_exists( 'ZipArchive' ) ) {
			die( esc_html__( 'ZIP extension is not installed, please install ZIP extension on your host or contact to your hosting provider and try again!', 'nevo-export' ) );
		}
 ...
 * @since    1.0.0
 */
if ( ! function_exists( 'nevo_export_create_data_files' ) ) {
	function nevo_export_create_data_files( $form_args ) {
		 ...
		 * Export hook
		 */
		do_action( 'nevo_export_before_create_data_files', $form_args );

		$content_data = array();
		WP_Filesystem();
		global $wp_filesystem;
		if ( ! file_exists( NEVO_EXPORT_TEMP ) ) {
			$wp_filesystem->mkdir( NEVO_EXPORT_TEMP );
		}
		if ( 1 == $form_args['include_media'] ) {
			if ( ! file_exists( NEVO_EXPORT_TEMP_UPLOADS ) ) {
				$wp_filesystem->mkdir( NEVO_EXPORT_TEMP_UPLOADS );
			}
		}
		...
		/*ignore post type*/
		$ignore_post_types = apply_filters( 'nevo_export_ignore_post_types', array( 'revision' ) );
		foreach ( $post_types as $post_type ) {
			...
			foreach ( $post_type_data as $single_post_data ) {
				...
				if ( 1 == $form_args['include_media'] && $post_type == 'attachment' ) {
					...
					if ( is_file( $file ) ) {
						if ( is_dir( NEVO_EXPORT_TEMP_UPLOADS ) ) {
							copy( $file, NEVO_EXPORT_TEMP_UPLOADS . basename( $file ) );
						}
					}
				}
				...
				/* $terms_data = array();
				foreach ($taxonomies as $taxonomy) {
					// Lấy tất cả các danh mục cho taxonomy hiện tại
					$terms_data[$taxonomy] = wp_get_post_terms($single_post_data->ID, $taxonomy, array('fields' => 'all'));
					if ($terms_data[$taxonomy]) {
						foreach ($terms_data[$taxonomy] as $tax_id => $single_term) {
							if (!empty($single_term->term_id)) {
								...
								// Lấy danh mục cha
								if ($single_term->parent) {
									$parent_term = get_term($single_term->parent, $taxonomy);
									if ($parent_term) {
										$terms_data[$taxonomy][$tax_id]->parent = array(
		...
		if ( 1 == $form_args['options_data'] ) {
			...
			$theme_option   = get_option( 'nevo_modules' );
			$options_data   = array();
			...
			$needed_options = apply_filters( 'nevo_export_include_options', $needed_options );

			foreach ( $all_options as $name => $value ) {
				if ( apply_filters( 'nevo_export_all_options', false ) ) {
		...
		if ( is_dir( NEVO_EXPORT_TEMP ) ) {

			/*content*/
			$wp_filesystem->put_contents( NEVO_EXPORT_TEMP . 'content.json', wp_json_encode( $content_data ) );
			...
			if ( 1 == $form_args['widgets_data'] ) {
				...
				$wp_filesystem->put_contents( NEVO_EXPORT_TEMP . 'widgets.json', wp_json_encode( $combine_widgets_data ) );
			}
			...
			if ( 1 == $form_args['options_data'] ) {
				...
				if ( ! empty( $theme_option ) ) {
					$combine_options_data['options']['nevo_modules'] = $theme_option;
				}
				$wp_filesystem->put_contents( NEVO_EXPORT_TEMP . 'options.json', wp_json_encode( $combine_options_data ) );
			}
		}
		nevo_export_create_zip( NEVO_EXPORT_TEMP, $wp_filesystem );
	}
}
if ( ! function_exists( 'nevo_export_ziparchive' ) ) {
	function nevo_export_ziparchive( $form_args ) {
		nevo_export_create_data_files( $form_args );
	}
}
